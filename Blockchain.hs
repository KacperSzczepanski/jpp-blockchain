...
mineBlock miner parent txs = findNonce 0 where
  cbase :: Transaction
  cbase = coinbaseTx miner

  header :: BlockHeader
  header = BlockHeader
    { parent = parent
    , coinbase = cbase
    , txroot = treeHash $ buildTree (cbase : txs)
    , nonce = 0
    }

  findNonce :: Word32 -> Block
  findNonce n = if validNonce header' then block else findNonce (n + 1) where
    header' :: BlockHeader
    header' = header { nonce = n }

    block :: Block
    block = Block { blockHdr = header', blockTxs = txs}
...
validChain blocks = if verifyChain blocks == Nothing then False else True

verifyChain :: [Block] -> Maybe Hash
verifyChain = foldr verify (Just 0) where
  verify :: Block -> Maybe Hash -> Maybe Hash
  verify _ Nothing = Nothing
  verify block (Just h) = verifyBlock block h
...
mineTransactions miner parent txs = (block, txrList) where
  block :: Block
  ...
  tree = buildTree (cbase : txs)

  txrList :: [TransactionReceipt]
  txrList = foldr (\tx acc -> TxReceipt { txrBlock = hash (blockHdr block), txrProof = (unpack $ buildProof tx tree)} : acc) [] txs

  unpack :: Maybe (MerkleProof Transaction) -> MerkleProof Transaction
  unpack Nothing = error "should never happen"
  unpack (Just proof) = proof
